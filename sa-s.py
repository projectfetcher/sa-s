...
subprocess.run([sys.executable, "-m", "pip", "install",
    ...
    "requests", "nest_asyncio", "tqdm", "-q"], check=True)
...
def print_live_stats():
    ...
    if s["strategy_hits"]:
        top = sorted(s["strategy_hits"].items(), key=lambda x: -x[1])[:5]
        strat_str = "  ".join(f"{k}:{v}" for k, v in top)
        print(f"  │  Strategies: {strat_str[:W-14]}")
    print(f"  └{'─'*W}┘")
...
async def main():
    ...
    if _stats["strategy_hits"]:
        print(f"  Top discovery strategies:")
        for strat, count in sorted(_stats["strategy_hits"].items(), key=lambda x: -x[1])[:10]:
    ...
    try:
        print(f"\n   Files saved to: {JOBS_FILE}  |  {COMPANIES_FILE}")
